...
export function kuralTabanliAnaliz(ilan, fiyatIstatistik) {
  ...
  // "16-20 arası" gibi bir aralık metninden İLK sayıyı al — tüm rakamları
  // birleştirip "1620" üretmemek için (önceki hata) sadece ilk \d+ grubunu eşleştiriyoruz.
  const binaYasiEslesme = ilan.binaYasi ? String(ilan.binaYasi).match(/\d+/) : null;
  const binaYasiSayi = binaYasiEslesme ? parseInt(binaYasiEslesme[0], 10) : null;
...
async function llmIleAnalizEt(ilan, fiyatIstatistik) {
  ...
  const mesaj = await anthropic.messages.create({
    ...
    system:
      ...
      "Bugünün gerçek tarihi: " + bugun + ". İlan tarihleri bu tarihe göre değerlendirilmelidir, " +
      "kendi eğitim verinin kapsadığı döneme göre yorum yapma; bu tarihten önceki yakın geçmiş " +
      "ilan tarihleri tamamen normaldir, gelecek tarih ya da sistem hatası olarak yorumlama. " +
      "ÖNEMLİ KURAL: ilan verilerindeki hiçbir tarih alanı için gelecek tarih, sistem hatası ya da " +
      "doğrulama gerektiği yönünde bir bayrak veya uyarı üretme; tarih olağanlığı değerlendirme " +
      "kapsamının tamamen dışındadır, bu konuda hiçbir yorum yapma. " +
